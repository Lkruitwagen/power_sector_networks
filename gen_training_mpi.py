...
def gen_training(pt,ID,q=None):
  ...
  try:
    ...
    #get the granule
    fname = get_granule(pt, download=True)
...
if (myrank==0):
  ...
  for pt in pts[(myrank-1)::(nprocs-1)]:
      print str(myrank)+" processing "+str(pt)
      gen_training(pt[0],pt[1])
